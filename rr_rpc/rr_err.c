...
void
rr_err_invalid_value(sr_session_ctx_t *ev_sess, const char *description, const char *bad_elem_name)
{
    if (bad_elem_name) {
        /* set error */
...
void
rr_err_sr2nc_get(sr_session_ctx_t *ev_sess, const sr_session_ctx_t *err_sess)
{
    ...
    if (strstr(err->message, " result is not a node set.")) {
        /* invalid-value */
        rr_err_invalid_value(ev_sess, err->message, NULL);
    } else {
        /* other error */
...
void
rr_err_sr2nc_edit(sr_session_ctx_t *ev_sess, const sr_session_ctx_t *err_sess)
{
    const sr_error_info_t *err_info;
    const sr_error_info_err_t *err;
    const char *ptr, *ptr2;
...
void
rr_err_sr2nc_same_ds(sr_session_ctx_t *ev_sess, const char *err_msg)
{
    /* set error */
...
void
rr_err_sr2nc_lock_denied(sr_session_ctx_t *ev_sess, const sr_error_info_t *err_info)
{
    struct rr_session *user_sess;
    const char *msg, *str, *ptr;
    char buf[11];

    SRPLG_LOG_DBG("rr-rpc", "prepare error message for lock");
    /* message */
    ...
    rr_get_sess_by_id(atoi(ptr + strlen(str)), &user_sess);

    sprintf(buf, "%" PRIu32, user_sess ? user_sess->id : 0);
